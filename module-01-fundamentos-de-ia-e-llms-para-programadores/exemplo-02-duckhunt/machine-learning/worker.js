...
async function loadModelAndLabels() {
    ...
    //A primeira rodada é mais lenta para carregar o modelo e otimizar a execução, as próximas rodadas serão mais rápidas
...
function preprocessImage(input) {
    return tf.tidy(() => {
        ...
        return tf.image.resizeBilinear(image, [INPUT_MODEL_DIMENTIONS, INPUT_MODEL_DIMENTIONS])
            .div(255)
            .expandDims(0);
    })
}
...
async function runInference(tensor) {
    ...
    //caixas(boxes), pontuaçãos(scores) e classes
...
function* processPredictions({ boxes, scores, classes }, width, height) {
    for (let index = 0; index < scores.length; index++) {
        ...
        //sempre de 4 em 4 posições temos as coordenadas de um bounding box (pq é um quadrado)
